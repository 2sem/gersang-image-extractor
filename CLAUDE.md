...
1. `handleFileSelect` / `selectTreeFile` — both call the shared `loadFile(file, displayLabel)` (async), which reads via `file.arrayBuffer()` → `parseSpriteFile`, and returns that promise so a caller (e.g. a search-result click) can `await` full rendering before acting further, like jumping to a specific frame.
...
### Item-name search (label catalog)

There's no metadata inside `.agf`/`.s32` files describing what an item actually is — filenames are cryptic (`armor_01_I.AGF`). `#treeSearchInput` (pinned above `#fileTree`) searches a community-maintained label catalog instead: `labels.js` declares an empty `GERSANG_LABELS` registry, and each cataloged item gets its own file under `indexes/` (e.g. `indexes/armor_01_I.js` does `GERSANG_LABELS["armor_01_I.AGF"] = { "1": "ABC 갑옷" }`), loaded via its own `<script>` tag in `index.html`'s `<head>` — one small file per item instead of one shared JSON avoids merge conflicts when multiple people contribute entries via separate PRs. There's deliberately no in-app editor; see `labels.js`'s header comment for the add-an-entry steps. All of this loads via plain `<script src>` tags rather than `fetch()`/XHR — browsers block `fetch()` of local files under `file://`, which would break this when `index.html` is opened directly instead of served over http(s); `<script src>` resource loading isn't subject to that restriction.

`handleFolderSelect` also builds `allTreeFiles` (a flat `[{name, file, relativePath}]`, alongside the nested tree) and caches the built tree DOM in `cachedTreeRootEl`. Typing in `#treeSearchInput` swaps `#fileTree`'s content between that cached tree (query empty) and `renderSearchResults(query)` (non-empty) — matching against `LABELS_LOOKUP` (a lowercase-keyed copy of `GERSANG_LABELS`, built once at script init) for per-frame label hits, and against the filename itself as a fallback so unlabeled files are still findable. Results are grouped by file with a `[인벤토리]`/`[필드]` badge (`getFileTypeBadge`, from the `_I`/`_F` filename suffix convention — a naming convention only, not something the format encodes), with matching frames listed underneath. Clicking a frame result awaits `loadFile` then calls `highlightFrame(frameIndex)`, which finds the `.image-item` whose `<label>` text matches, scrolls it into view, and pulses it via the `.search-highlight` class (`@keyframes search-pulse`).
