...
There's no metadata inside `.agf`/`.s32` files describing what an item actually is — filenames are cryptic (`armor_01_I.AGF`). The sidebar is a **static reference guide** built from a community-maintained label catalog: `labels.js` declares an empty `GERSANG_LABELS` registry, and each cataloged item gets its own file under `indexes/` (e.g. `indexes/armor_01_I.js` does `GERSANG_LABELS["armor_01_I.AGF"] = { "1": "ABC 갑옷" }`), loaded via its own `<script>` tag in `index.html`'s `<head>` — one small file per item instead of one shared JSON avoids merge conflicts when multiple people contribute entries via separate PRs. There's deliberately no in-app editor; see `labels.js`'s header comment for the add-an-entry steps. All of this loads via plain `<script src>` tags rather than `fetch()`/XHR — browsers block `fetch()` of local files under `file://`, which would break this when `index.html` is opened directly instead of served over http(s); `<script src>` resource loading isn't subject to that restriction.

**Important: this guide is not tied to any locally picked file, and its entries are not clickable.** `renderLabelGuide(query)` renders directly from `GERSANG_LABELS` — with `query` empty (page load, or the search box cleared) it lists every cataloged file; typing filters by matching either a per-frame label or the filename itself. Results are grouped by file with a `[인벤토리]`/`[필드]` badge (`getFileTypeBadge`, from the `_I`/`_F` filename suffix convention — a naming convention only, not something the format encodes), with matching frames listed underneath as plain text. The guide's job is to tell the user which filename to go open via the `#fileInput` picker in the toolbar — it never calls `loadFile` itself. (An earlier version of this feature mirrored a locally-picked folder via `webkitdirectory` with click-to-load; that was removed because the sidebar's purpose is a global reference guide, not a per-session file browser.)
