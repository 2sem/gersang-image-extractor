...
1. Read the file into an `ArrayBuffer` (`file.arrayBuffer()`).
...
Filenames like `dress02_i.AGF` don't tell you what the item actually is, and there's no such metadata inside `.agf`/`.s32` files themselves. The sidebar is a **static reference guide**, not a file browser: it lists a community-maintained catalog of "which file (and frame) has which item," independent of anything you've loaded, and lets you search it. It does not open files — its job is to tell you which filename to go pick from the "파일 선택" button above.

- `labels.js` declares an empty registry; each cataloged sprite file gets its own tiny source file under `indexes/` (nested however you like, e.g. `indexes/Textures/Items/dress02_i.js` does `GERSANG_LABELS["dress02_i.AGF"] = { name: "의복2", items: { "22": "복희의 의복", "23": "츠쿠요미 의복" } }` — `name` is an optional friendly display name for the file, `items` maps frame number → item name). One small file per sprite file — rather than one shared JSON blob — means adding an entry is a tiny diff with no merge-conflict risk when several people contribute via separate PRs.
- Those per-item files are only for editing. `python3 scripts/build-indexes.py` concatenates every `indexes/**/*.js` into one generated `indexes.js`, which is what `index.html` actually loads via a single fixed `<script>` tag — so cataloging a new item never means editing `index.html` (there's no manifest to maintain either, however many items get added). The build script also derives each entry's `.folder` from where its source file lives under `indexes/` (e.g. `Textures/Items` for the file above) — contributors never set this themselves. There's no in-app editor: to add an entry, create/edit a file under `indexes/`, run the build script, and commit both the source file and the regenerated `indexes.js` in a PR.
- The sidebar always shows a collapsible tree grouped by that `.folder` path (mirroring `indexes/`'s own layout), so browsing feels like exploring the game's asset folders. With the search box empty, the tree holds the whole catalog; typing filters it down to matches while still showing their parent folders, matching an item name, the file's friendly name, or the filename itself — matching at the item level shows just those items, matching only via the filename/name shows every item in that file for context. Entries are tagged `[인벤토리]`/`[필드]` from the `_I`/`_F` filename suffix convention (inventory icon vs. on-ground/field sprite — a naming convention, not something the file format encodes).
- Entries never open or load a file on click. Clicking an item row does one thing: if that item's file is the one currently loaded and the frame is actually visible in the gallery, it scrolls to and briefly flashes that thumbnail. Otherwise it just tells you (via the status line) which file to go open via "파일 선택" — same as if it did nothing, plus a hint.
- Once you *do* open a cataloged file, the same catalog shows up right in the gallery: any thumbnail whose frame number has a cataloged item name shows that name underneath it, so you don't need to cross-reference back to the sidebar. Uncataloged frames just show their frame number, same as before.
